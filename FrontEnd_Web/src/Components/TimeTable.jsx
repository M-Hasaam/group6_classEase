...
function FormSection({
  ...
  const batchOptions = department ? cleanOptions(getBatchYears()) : [];
  ...
  useEffect(() => {
    ...
    if (program && rawDept.length > 0 && rawDept.every((o) => o.toLowerCase() === "null")) {
      setDepartment("null");
    }
  }, [program]);
  ...
  useEffect(() => {
    ...
    if (department && rawBatch.length > 0 && rawBatch.every((o) => o.toLowerCase() === "null")) {
      setBatchYear("null");
    }
  }, [department]);
  ...
  useEffect(() => {
    ...
    if (batchYear && rawSec.length > 0 && rawSec.every((o) => o.toLowerCase() === "null")) {
      setSection("null");
    }
  }, [batchYear]);
...
const FASTTimetable = () => {
  ...
  const getPrograms = () =>
    Object.keys(timetableData).filter((k) => !["Update", "Other", "PHD"].includes(k));

  const getDepartments = () =>
    program && timetableData[program] ? Object.keys(timetableData[program]) : [];

  const getBatchYears = () =>
    program && department ? Object.keys(timetableData[program][department] || {}) : [];
  ...
  /* ---------------------------------------------------
     ...
     {
       main: [slot, ...],
       subsections: { subName: [slot, ...], ... }
     }
     Each slot: { time: {...}, classes: [...] }
     Duplicate removal applied per (main / each subsection)
  ---------------------------------------------------- */
  const getDayData = (selectedDay) => {
    ...
    // helper to combine and dedupe an array of slots
    const dedupeSlots = (slotsArray) => {
      // We'll dedupe classes inside each slot and also dedupe identical slot+class combos
    ...
    // MAIN: combine mainClasses + nullClasses for selectedDay
    const mainRaw = [...(sectionData[selectedDay] || []), ...(nullSection[selectedDay] || [])];
    const mainUnique = dedupeSlots(mainRaw);

    // SUBSECTIONS: sectionData.Subsections expected as { subName: { Monday: [...], ... }, ... }
    ...
    for (const [subName, subObj] of Object.entries(subsectionsRaw)) {
      // subObj is expected to have day keys
      const subDayArr = subObj[selectedDay] || [];
      if (!subDayArr || subDayArr.length === 0) continue;

      // dedupe classes for this subsection (fresh seen set so subsections don't dedupe against main)
      const deduped = (() => {
        const seen = new Set();
        const res = [];
  ...
  /* ---------------------------------------------------
     ...
     For Whole Week, we render each day with main + subsections
  ---------------------------------------------------- */
  const renderTimetable = () => {
              ...
              {sortedMain.map((slot, i) => (
                ...
                >
                  <div className="text-blue-300 font-semibold">
                    {slot.time.start.text} {slot.time.start.AM_PM} - {slot.time.end.text}{" "}
                    {slot.time.end.AM_PM}
                  </div>
                  {slot.classes.map((cls, idx) => (
                    <div key={idx} className="mt-2 text-slate-200">
                      ...
                      <div className="text-xs mt-1 text-slate-400">{cls.type}</div>
              ...
              return (
                <div key={`sub-${subName}`} className="mb-6">
                  <div className="bg-slate-700 px-4 py-2 rounded-lg mb-2">
                    <h4 className="text-blue-300 font-semibold">Subsection {subName}</h4>
                  </div>
    ...
    if (day === "Whole Week") {
      ...
      const weekBlocks = daysOrder.map((d) => renderDayBlock(d)).filter(Boolean);
